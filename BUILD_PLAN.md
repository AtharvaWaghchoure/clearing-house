...
- [x] Issue the bond via the **real ATS Factory** `0.0.9213391` — live diamond `0x6e19…522a` (`0.0.10418524`), `name()`="HELVETIA 4.25% 15FEB2031"; `contracts/src/interfaces/ats/IATSFactory.sol` + `verifier/src/deploy/ats-bond.ts`.
- [x] **Settle that real diamond through the venue** — `verifier/src/deploy/ats-settle.ts`: roles + `addIssuer` + `grantKyc` + `issue` + hold, then `engine.settle` → buyer holds 10 of the ATS bond, delivery via a real ATS hold (settle tx `0xf85dffd7…`, `SUCCESS`). Every hold/compliance signature matched our `IATSSecurity` byte-for-byte; the venue contracts didn't change. Surfaced + fixed a real mock/real gap (live ATS `canTransferByPartition` rejects zero `_from` + does a free-balance check) — leg + engine re-verified exact-match.
